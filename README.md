...
3. **完整的双人联机入口：** 基于 OnlineSubsystem 实现创建、搜索、加入和销毁房间，并处理异步操作冲突、连接失败、多人地图切换和 Host 存活期间的客户端重新接入。
...
- 加入选中的房间。
- 销毁已有房间。
- 缓存客户端最后一次成功加入的会话，并提供重新接入接口。
- 记录连接和地图加载失败。
- 防止创建、搜索、加入和重新接入等异步操作互相重叠。

Host 创建房间成功后，服务器切换到合作地图，并带上已经连接的玩家。Client 加入成功后，根据 OnlineSubsystem 返回的地址连接服务器。

客户端成功加入后，GameInstance 会保留上一次会话结果。网络中断时不会清除该结果；重新接入会先销毁客户端本地残留会话，再重新调用 `JoinSession`，成功解析地址后通过 `ClientTravel` 返回仍在运行的 Host。主动退出房间或改为创建房间时会清除旧记录。

对外只提供 `CanReconnectLastSession`、`ReconnectLastSession` 和 `OnReconnectComplete`，菜单负责按钮与提示，角色和机关不依赖会话实现。服务器通过 GameMode 的 `PostLogin`、`Logout` 输出连接人数、共享进度和胜利状态，便于核对玩家离开后的机关清理与重新接入结果。
...
```text
客户端网络中断
→ GameInstance 保留最后一次成功会话
→ 清理客户端本地残留会话
→ 重新 JoinSession
→ 解析服务器地址
→ ClientTravel 返回仍在运行的 Host
```
